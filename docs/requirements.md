...
スマートフォンで対応ブラウザを使用している場合、端末のモーションセンサー入力に合わせてCanvas表示を一時的に揺らし、パターンを手元で動かしている感覚を追加する。
...
- Canvas内部のシミュレーション状態やPNG保存内容は変更しない
- 揺れ量は上限を設け、画面操作を妨げない範囲に抑える
...
- 対応スマートフォンでのモーションセンサーによるCanvas表示揺れ
